...
public class RunecrafterAbstractScript extends AbstractScript {
    private enum State {WALK_TO_BANK, BANKING, WALK_TO_ALTAR, USE_RUINS, CRAFT, LEAVE_RUINS, BUY_RUNES}
    ...
    private int buy_runes() {
        if (GrandExchange.open()) {
            if (!placedOrder) {
                ...
                openSlot = GrandExchange.getFirstOpenSlot();
                GrandExchange.buyItem("Pure essence", runesToBuy, price);
                placedOrder = true;
            } else {
                if (Sleep.sleepUntil(() -> GrandExchange.isReadyToCollect(openSlot), 15000)) {
                    placedOrder = false;
